...
Take a look at both `SumAll` and `SumAllTails` that we wrote in [arrays and slices](fruit/Coding/Testing/go/fundamentals/arrays-and-slices.md). If you don't have your version please copy the code from the [arrays and slices](fruit/Coding/Testing/go/fundamentals/arrays-and-slices.md) chapter along with the tests.
...
Go has always had higher-order functions, and as of version 1.18 it also has [generics](fruit/Coding/Testing/go/fundamentals/generics.md), so it is now possible to define some of these functions discussed in our wider field. There's no point burying your head in the sand, this is a very common abstraction outside the Go ecosystem and it'll be beneficial to understand it.
...
You should be familiar with the generics syntax [from the previous chapter](fruit/Coding/Testing/go/fundamentals/generics.md), try and write your own `Reduce` function and use it inside `Sum` and `SumAllTails`.
