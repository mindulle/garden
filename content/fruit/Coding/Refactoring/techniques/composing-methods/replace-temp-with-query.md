---
description: "Problem: You place the result of an expression in a local
  variable for later use in your code. Solution: Move the entire
  expression to a separate method and return the result from it. Query
  the method instead of using a variable. Incorporate the new method in
...
Method](/extract-method) for a portion of a very long method.
...
1.  Make sure that a value is assigned to the variable once and only
    ...
    Variable](/split-temporary-variable) to ensure that the variable
    will be used only to store the result of your expression.

2.  Use [Extract Method](/extract-method) to place the expression of
...
.fa-arrow-right}](/split-temporary-variable){.btn .btn-primary
rel="next"}
...
[[]{.fa .fa-arrow-left} Inline Temp](/inline-temp){.btn .btn-default
rel="prev"}
...
-    [English]{.caption .d-none .d-lg-inline-block}
    ...
    [English](https://refactoring.guru/replace-temp-with-query "English"){.dropdown-item
    .locale-link .active locale="en"}
    [Español](https://refactoring.guru/es/replace-temp-with-query "Español"){.dropdown-item
    .locale-link locale="es"}
    [Français](https://refactoring.guru/fr/replace-temp-with-query "Français"){.dropdown-item
    .locale-link locale="fr"}
    [日本語](https://refactoring.guru/ja/replace-temp-with-query "日本語"){.dropdown-item
    .locale-link locale="ja"}
    [한국어](https://refactoring.guru/ko/replace-temp-with-query "한국어"){.dropdown-item
    .locale-link locale="ko"}
    [Polski](https://refactoring.guru/pl/replace-temp-with-query "Polski"){.dropdown-item
    .locale-link locale="pl"} [Português
    Brasileiro](https://refactoring.guru/pt-br/replace-temp-with-query "Português Brasileiro"){.dropdown-item
    .locale-link locale="pt-br"}
    [Русский](https://refactoring.guru/ru/replace-temp-with-query "Русский"){.dropdown-item
    .locale-link locale="ru"}
    [Українська](https://refactoring.guru/uk/replace-temp-with-query "Українська"){.dropdown-item
    .locale-link locale="uk"}
    [中文](https://refactoringguru.cn/replace-temp-with-query "中文"){.dropdown-item
