---
description: "Problem: A class has features that are used only in
  certain cases. Solution: Create a subclass and use it in these cases."
lang: en
theme-color: "#95f3fc"
...
::: ba-container
![Extract Subclass -
Before](/images/refactoring/diagrams/Extract%20Subclass%20-%20Before.png?id=ea6e8c170ae321116dbf645e0db5b0f0){width="209"}
:::
:::
...
::: ba-container
![Extract Subclass -
After](/images/refactoring/diagrams/Extract%20Subclass%20-%20After.png?id=caff9775084405e0e46aec91bb2d4feb){width="209"}
:::
:::
:::
:::
...
-   Despite its seeming simplicity, *Inheritance* can lead to a dead end
    ...
    the [Strategy](/design-patterns/strategy) pattern). In other words,
    the `Dog` class will have two component fields, size and fur. You
    will plug in component objects from the necessary classes into these
    fields. So you can create a `Dog` that has `LargeSize` and
    `ShaggyFur`.
...
4.  Move the necessary methods and fields from the parent class to the
    subclass. Do this via [Push Down Method](/push-down-method) and
    [Push Down Field](/push-down-field). It's simpler to start by moving
    the methods first. This way, the fields remain accessible throughout
    the whole process: from the parent class prior to the move, and from
    the subclass itself after the move is complete.
...
[Extract Superclass []{.fa .fa-arrow-right}](/extract-superclass){.btn
...
[[]{.fa .fa-arrow-left} Push Down Field](/push-down-field){.btn
...
-    [English]{.caption .d-none .d-lg-inline-block}
    ...
    [English](https://refactoring.guru/extract-subclass "English"){.dropdown-item
    .locale-link .active locale="en"}
    [Español](https://refactoring.guru/es/extract-subclass "Español"){.dropdown-item
    .locale-link locale="es"}
    [Français](https://refactoring.guru/fr/extract-subclass "Français"){.dropdown-item
    .locale-link locale="fr"}
    [日本語](https://refactoring.guru/ja/extract-subclass "日本語"){.dropdown-item
    .locale-link locale="ja"}
    [한국어](https://refactoring.guru/ko/extract-subclass "한국어"){.dropdown-item
    .locale-link locale="ko"}
    [Polski](https://refactoring.guru/pl/extract-subclass "Polski"){.dropdown-item
    .locale-link locale="pl"} [Português
    Brasileiro](https://refactoring.guru/pt-br/extract-subclass "Português Brasileiro"){.dropdown-item
    .locale-link locale="pt-br"}
    [Русский](https://refactoring.guru/ru/extract-subclass "Русский"){.dropdown-item
    .locale-link locale="ru"}
    [Українська](https://refactoring.guru/uk/extract-subclass "Українська"){.dropdown-item
    .locale-link locale="uk"}
    [中文](https://refactoringguru.cn/extract-subclass "中文"){.dropdown-item
